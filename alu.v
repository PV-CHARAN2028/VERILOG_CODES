...
module alu(input [7:0]a_in,b_in,
   ...
   always@(command_in)
	 ...
	 case(command_in)
		 ...
		 INC  :out =a_in+1'b1;                    // Increment a by 1. 
		 SUB  :out =a_in-b_in;                    // Subtracts b from a.
		 DEC  :out =a_in-1'b1;                    // Decrement a by 1.
		 MUL  :out =a_in*b_in;                    // Multiply 4 bit numbers a and b.
		 DIV  :out =a_in/b_in;                    // Divide a by b.
		 SHL  :out = a_in<<1;                     //shift a to left by 1'b1. 
		 SHR  :out =a_in>>1;                      //shift a to right by 1'b1.
		 AND  :out =a_in&b_in;                    //logical and(&) operation.
		 OR   :out =a_in|b_in;                    //logical or(|) operaion.
		 INV  :out =!a_in;                        //bitwise inverse operation.
		 NAND :out =~(a_in&b_in);                 //bitwise nand operation .
		 NOR  :out =~(a_in|b_in);                 //bitwise nor operation .
		 XOR  :out = a_in^b_in;                   //bitwise xor operation.
		 XNOR :out =~(a_in^b_in);                 //bitwise xnor oeration.
		 BUF  :out  = a_in;                       //buffer.
	 endcase
      end
